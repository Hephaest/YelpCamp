...
const createCampground = async (req, res, next) => {
    ...
    const images = req.files.map(f => new Image({ url: f.path, filename: f.filename }));
    ...
    successRedirect(req, res, successMsg.CREATE_SUCCESS, detailUrl(campground._id));
};
...
const updateCampground = async (req, res) => {
    const { id } = req.params;
    const campground = await Campground.findByIdAndUpdate(id, { ...req.body.campground }).populate('images');
    const images = req.files.map(f => new Image({ url: f.path, filename: f.filename }));
    campground.images.push(...images);
    ...
    successRedirect(req, res, successMsg.UPDATE_SUCCESS, detailUrl(campground._id));
};
