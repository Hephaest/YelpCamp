...
const addPageItems = (el, start, end) => {
    for (let idx = start; idx <= end; idx ++) {
        el.innerHTML += `<li class="page-item ${ currentPage == idx ? 'active' : '' }"><a class="page-link" href="/campgrounds/?page=${ idx }&sort=${ currentSort }" >${ idx }</a></li>`
    }
};

const addHintItem = (el, title, status) => {
    el.innerHTML += `<li class="page-item ${ status == 'disabled' ? 'disabled' : '' }">
    <a class="page-link" href="${ status == 'disabled' ? '#' : '/campgrounds/?page=' + (currentPage + (title == 'Next'? 1 : -1)) + '&sort=' + currentSort }" >
    ${ title }
    </a>
    </li>`;
};
...
document.addEventListener( "DOMContentLoaded", () => {
    let wrappers = document.querySelectorAll('.truncate-text');
    let options = { height: 16 * 6 };
    for (let wrapper of wrappers) new Dotdotdot( wrapper, options );
});

/***************************** Button Listener *************************/
const goDetailBtns = document.querySelectorAll('.view-detail');

for (let btn of goDetailBtns) {
